...
async function loadBatchHistory() {
    try {
        ...
        if (!data.videos || data.videos.length === 0) {
            list.innerHTML = '<p style="color:var(--text-dim);padding:16px;text-align:center;">暂无下载记录</p>';
        } else {
            list.innerHTML = data.videos.map(v => `
                <div class="history-item">
                    ...
                    <button class="btn btn-sm" onclick="deleteHistoryVideo('${v.filename}', this)" style="color:#ff6b6b;">✕</button>
                </div>
            `).join('');
        }
    } catch (e) { /* ignore */ }
...
async function deleteHistoryVideo(filename, btn) {
